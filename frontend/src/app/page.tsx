...
export default function Home() {
    ...
    if (!isAuthReady) {
        return (
            <div className="min-h-screen bg-teal-950 text-teal-50 flex items-center justify-center">
                <span className="w-10 h-10 border-4 border-teal-800 border-t-teal-400 rounded-full animate-spin"></span>
            </div>
        );
    }
    ...
    if (!authUser) {
        return (
            <div className="min-h-screen bg-teal-950 text-teal-50 flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full blur-[120px] pointer-events-none bg-teal-500/10" />
                <div className="max-w-md w-full bg-teal-900/40 backdrop-blur-3xl border border-teal-800/50 p-10 rounded-[2rem] shadow-2xl relative z-10 text-center">
                    <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent pointer-events-none rounded-[2rem]" />
                    <div className="relative z-10">
                        <Image src="/logo.png" alt="HandyMate Logo" width={100} height={100} className="mx-auto mb-6 drop-shadow-lg rounded-2xl" />
                        <h1 className="text-3xl font-extrabold tracking-tight mb-2">
                            <span className="text-white">Handy</span><span className="text-teal-400">Mate</span>
                        </h1>
                        <p className="text-teal-200/70 mb-10 font-medium leading-relaxed">
                            Your personal AI-powered DIY contractor. Sign in to save your tool inventory and project history securely.
                        </p>
                        <button 
                            onClick={async () => {
                                try {
                            ...
                            className="w-full py-4 bg-teal-500 hover:bg-teal-400 text-teal-950 font-black rounded-2xl shadow-[0_0_30px_rgba(20,184,166,0.3)] transition-all hover:-translate-y-1 flex items-center justify-center gap-3"
                        >
                ...
                <div className="p-6 flex-1 overflow-y-auto relative z-10">
                    <div className="mb-8">
                        ...
                        <div className="bg-teal-950/50 rounded-xl p-4 border border-teal-800/50 shadow-inner">
                            ...
                            {authUser && (
                                <>
                                    <div className="text-xs text-teal-300/70 mb-1 border-t border-teal-800/50 pt-3">Authenticated As:</div>
                                    <div className="font-medium text-xs text-teal-100 truncate mb-3">{authUser.email}</div>
                                    <button 
                                        onClick={async () => {
                                            try {
                                                await logout();
                                        ...
                                        className="w-full py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 rounded-lg text-xs font-bold transition-colors"
                                    >
                                        Sign Out
                                    </button>
                                </>
